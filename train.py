...
Evolved from Karpathy's nanoGPT with:
  - Chuck Optimizer (self-aware AdamW) replacing blind Adam
  - notorch philosophy: understand what you run
  - Dracula dataset: because enough Shakespeare

Single GPU / CPU training:
$ python train.py --batch_size=32 --compile=False --device=cpu

With DDP on multiple GPUs:
...
# Chuck Optimizer awareness
try:
    from ariannamethod import ChuckOptimizer
    CHUCK_AVAILABLE = True
except ImportError:
    CHUCK_AVAILABLE = False

# -----------------------------------------------------------------------------
# default config values — tuned for dracula.txt character-level training
# I/O
out_dir = 'weights'
eval_interval = 250
log_interval = 1
...
# optimizer — Chuck sees, Chuck remembers
optimizer = model.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
is_chuck = isinstance(optimizer, ChuckOptimizer) if CHUCK_AVAILABLE else False
print(f"optimizer: {'Chuck' if is_chuck else 'AdamW'}")
...
while True:
    ...
    # step the optimizer — Chuck gets the loss for self-awareness
    if is_chuck:
        scaler.step(optimizer, loss=loss.item() * gradient_accumulation_steps)
    else:
