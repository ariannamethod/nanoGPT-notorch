/*
 * train_nanogpt.c — nanoGPT on notorch. 10.2M char-level on Dracula.
 *
 * Architecture: dim=320, L=8, H=8, HD=40, FFN=896, V=94, CTX=256, RoPE, MHA
 * Dataset: dracula.txt (843KB, Bram Stoker)
 * Karpathy formula: ~0.84MB → 10-12K steps
 *
 * Build: cc -O2 -I ariannamethod train_nanogpt.c ariannamethod/notorch.c -o train_nanogpt -lm
 * macOS: add -framework Accelerate -DUSE_BLAS -DACCELERATE -DACCELERATE_NEW_LAPACK
 * Run:   ./train_nanogpt [steps] [lr]
 *        ./train_nanogpt --resume [steps] [lr]
 */
...
#define CKPT_PREFIX "nanogpt_ckpt"

/* ── Vocabulary: 94 printable chars from Dracula ── */
static char vocab_chars[VOCAB];
static int  char_to_id[256];

static void init_vocab(const unsigned char* data, long fsize) {
    memset(char_to_id, -1, sizeof(char_to_id));
    int idx = 0;
    /* Scan dataset for unique chars, sorted */
    int seen[256] = {0};
    for (long i = 0; i < fsize; i++) seen[data[i]] = 1;
    for (int c = 0; c < 256 && idx < VOCAB; c++) {
        if (seen[c]) {
            vocab_chars[idx] = (char)c;
            char_to_id[c] = idx;
            idx++;
        }
    }
    printf("vocab: %d chars\n", idx);
}
...
static void save_model(Model* m, const char* prefix) {
    ...
    nt_tensor** p = model_param_array(m); nt_save(path, p, model_n_tensors()); free(p);
}

static void save_checkpoint(Model* m, int step, float best_loss) {
    save_model(m, CKPT_PREFIX);
    char mpath[256]; snprintf(mpath, sizeof(mpath), "%s.meta", CKPT_PREFIX);
...
int main(int argc, char** argv) {
    ...
    init_vocab(raw, fsize);

    int* encoded = (int*)malloc(fsize * sizeof(int));
    for (long i = 0; i < fsize; i++) {
        int id = char_to_id[raw[i]];
        encoded[i] = id >= 0 ? id : 0;
    }
    free(raw);
    printf("corpus: %.1f KB (%ld chars, vocab %d)\n", fsize/1024.0, fsize, VOCAB);

    nt_seed(42);
    Model* model = model_new();
    long np = count_params(model);
    printf("model: %ld params (%.1f MB)\n", np, np*4.0f/1048576.0f);
    printf("karpathy: %.1fKB data, %.1fM params, %d steps (%.1f epochs)\n",
           fsize/1024.0, np/1e6, steps, (float)steps * CTX / fsize);

    int start_step = 0;
    float best_loss = 99.0f;
    if (resume) {
        int loaded = load_checkpoint(model, &best_loss);
        if (loaded >= 0) { start_step = loaded; printf("RESUMED step %d, best %.4f\n", start_step, best_loss); }
        else printf("no checkpoint, starting fresh\n");
    }
    ...
    for (int step = start_step; step < steps; step++) {
        ...
        if ((step+1) % CKPT_EVERY == 0 && step > start_step) {
            float val = eval_loss(model, encoded, fsize);
    ...
    float final_val = eval_loss(model, encoded, fsize);
    ...
    for (int p = 0; p < 3; p++) {
        ...
        for (int i = 0; seed[i] && gen_len < CTX/2; i++) {
            int id = char_to_id[(unsigned char)seed[i]];
            ctx[gen_len++] = id >= 0 ? id : 0;
        }
        printf("%s", seed);
        for (int s = 0; s < 200; s++) {
    ...
    /* Save */
    printf("── saving ──\n");
    save_model(model, "nanogpt_dracula");
    printf("  nanogpt_dracula.bin (%.1f MB)\n", np*4.0f/1048576.0f);
    save_checkpoint(model, steps, best_loss);

    model_free(model); free(encoded);
    printf("\n════════════════════════════════════════════════════════\n");
    printf("  nanoGPT 10.2M trained on Dracula. No Python. No PyTorch.\n");
    printf("  Chuck approves. Karpathy liberated.\n");
    printf("════════════════════════════════════════════════════════\n");
    return 0;
}
